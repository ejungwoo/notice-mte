...
int main(int argc, char *argv[])
{
  ...
  unsigned long adc_peak_sum_width;
  unsigned long adc_peak_sum_delay;
  unsigned long adc_peak_mode;
  unsigned long adc_fast_daq;
  int ch;
  ...
  fscanf(fp, "%ld", &adc_peak_sum_width);
  fscanf(fp, "%ld", &adc_peak_sum_delay);
  fscanf(fp, "%ld", &adc_peak_mode);
  fscanf(fp, "%ld", &adc_fast_daq);
  fclose(fp);
...
//i    CLOVERinit_DAC(sid, mid[adc]);
    CLOVERwrite_CW(sid, mid[adc], adc_cw);
    CLOVERwrite_POL(sid, mid[adc], adc_pol); 
    CLOVERwrite_MTHR(sid, mid[adc], adc_mthr);
    CLOVERset_TRAPEZOIDAL_FILTER(sid, mid[adc], 0, adc_rise_time, adc_flat_top, adc_decay_time);
    ...
    CLOVERwrite_PEAKSUM_WIDTH(sid, mid[adc], 0, adc_peak_sum_width);
    CLOVERwrite_PEAKSUM_DLY(sid, mid[adc], 0, adc_peak_sum_delay);
    CLOVERwrite_PEAK_MODE(sid, mid[adc], adc_peak_mode);
    CLOVERwrite_ENABLE_FASTDAQ(sid, mid[adc], adc_fast_daq);
  }
  ...
  for (adc = 0; adc < num_of_adc; adc++) {
    ...
    printf("ADC@%d rise time = %ld ns\n", mid[adc], CLOVERread_RISE_TIME(sid, mid[adc], 1)); 
    printf("ADC@%d flat top = %ld ns\n", mid[adc], CLOVERread_FLAT_TOP(sid, mid[adc], 1));
    printf("ADC@%d decay time = %ld ns\n", mid[adc], CLOVERread_DECAY_TIME(sid, mid[adc], 1));
    ...
    printf("ADC@%d ch%d threshold = %ld\n", mid[adc], ch, CLOVERread_THR(sid, mid[adc], 1));
    ...
    printf("ADC@%d peak sum width = %ld ns\n", mid[adc], CLOVERread_PEAKSUM_WIDTH(sid, mid[adc], 1));
    printf("ADC@%d peak sum delay = %ld ns\n", mid[adc], CLOVERread_PEAKSUM_DLY(sid, mid[adc], 1));
    printf("ADC@%d peak sum mode = %ld\n", mid[adc], CLOVERread_PEAK_MODE(sid, mid[adc]));
    printf("ADC@%d fast DAQ enable = %ld\n", mid[adc], CLOVERread_ENABLE_FASTDAQ(sid, mid[adc]));
